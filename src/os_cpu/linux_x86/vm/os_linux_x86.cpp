...
JVM_handle_linux_signal(int sig,
  ...
  if (os::Linux::signal_handlers_are_installed) {
    if (t != NULL ){
      ...
      /* When we sample the call stacks using a timer-driven signal, we
       * handle segfaults generated by the sampling by just checking if they
       * originated from the WatcherThread. XXX: We should make this check
       * stronger to ensure the segfault is due to sampling and not some other
       * use of the watcher thread.
       */
      else if (t->is_Watcher_thread() && sig == SIGSEGV) {
        HotMethodSampler::segfault_samples++;
        siglongjmp(*env, 1);
        return true;
      }
      else if (t->is_Sampler_thread() && sig == SIGSEGV) {
        HotMethodSampler::segfault_samples++;
        siglongjmp(*env, 1);
        return true;
      }
               ...
               ObjectInfoCollection::collecting() &&
               sig == SIGSEGV) {
        HotMethodSampler::segfault_samples++;
        siglongjmp(*env, 1);
        return true;
