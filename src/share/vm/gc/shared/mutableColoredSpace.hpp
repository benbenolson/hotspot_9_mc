...
#ifndef SHARE_VM_GC_SHARED_MUTABLECOLOREDSPACE_HPP
#define SHARE_VM_GC_SHARED_MUTABLECOLOREDSPACE_HPP
...
class MutableColoredSpace : public MutableSpace {
  ...
  class ColoredSpace : public CHeapObj {
   ...
   public:
    ColoredSpace(HeapColor c, size_t alignment) : _color(c), _last_page_scanned(NULL), _allocation_failed(false) {
      _space = new MutableSpace(alignment);
      _alloc_rate = new AdaptiveWeightedAverage(ColoredSpaceChunkResizeWeight);
    }
    ~ColoredSpace() {
      delete _space;
      delete _alloc_rate;
    ...
    static bool equals(void* color_id_value, ColoredSpace* p) {
      return *(HeapColor*)color_id_value == p->color();
    }
    ...
    HeapColor color() const                         { return _color;               }
  ...
  GrowableArray<ColoredSpace*>* _colored_spaces;
  bool _is_eden;
  size_t _page_size;
  ...
  size_t _total_committed_space_size;
  void set_committed_space_size(size_t v)            { _total_committed_space_size = v;      }
  size_t total_committed_space_size() const          { return _total_committed_space_size;   }
  ...
  size_t default_chunk_size(MutableColoredSpace *mcs, int i);
  ...
  void colored_initialize(MemRegion mr, MutableColoredSpace *mcs,
                          bool clear_space, bool mangle_space,
                          bool setup_pages = SetupPages);

 public:
  GrowableArray<ColoredSpace*>* colored_spaces() const  { return _colored_spaces; }
  MutableColoredSpace(size_t alignment, bool is_eden=false);
  virtual ~MutableColoredSpace();
  ...
  virtual size_t capacity_in_words(HeapColor color) const;
  ...
  virtual HeapWord* allocate(size_t word_size) {
    return allocate(word_size, UnknownObjectHeapColor);
  }
  virtual HeapWord* cas_allocate(size_t word_size) {
    return cas_allocate(word_size, UnknownObjectHeapColor);
  }
  ...
  void resize_colored_space(MemRegion mr);
  void move_red_to_blue();
};

#endif // SHARE_VM_GC_IMPLEMENTATION_SHARED_MUTABLECOLOREDSPACE_HPP
