...
/* MRJ - for moving colored objects */
#include "gc/parallel/psScavenge.hpp"


MutableColoredSpace::MutableColoredSpace(size_t alignment, bool is_eden) :
  _is_eden(is_eden), MutableSpace(alignment) {
  _page_size = os::vm_page_size();
  _adaptation_cycles = 0;
  _samples_count = 0;

  _colored_spaces = new (ResourceObj::C_HEAP, mtInternal) GrowableArray<ColoredSpace*>(0, true);

  for (int i = 0; i < HC_TOTAL; i++) {
    HeapColor c = static_cast<HeapColor>(i);
    _colored_spaces->append(new ColoredSpace(c, alignment));
  }
}

MutableColoredSpace::~MutableColoredSpace() {
  for (int i = 0; i < colored_spaces()->length(); i++) {
    delete colored_spaces()->at(i);
  }
  delete colored_spaces();
}

#ifndef PRODUCT
void MutableColoredSpace::mangle_unused_area() {
  // This method should do nothing.
  // It can be called on a numa space during a full compaction.
}
void MutableColoredSpace::mangle_unused_area_complete() {
  // This method should do nothing.
  // It can be called on a numa space during a full compaction.
}
void MutableColoredSpace::mangle_region(MemRegion mr) {
  // This method should do nothing because numa spaces are not mangled.
}
void MutableColoredSpace::set_top_for_allocations(HeapWord* v) {
  assert(false, "Do not mangle MutableColoredSpace's");
}
void MutableColoredSpace::set_top_for_allocations() {
  // This method should do nothing.
}
void MutableColoredSpace::check_mangled_unused_area(HeapWord* limit) {
  // This method should do nothing.
}
void MutableColoredSpace::check_mangled_unused_area_complete() {
  // This method should do nothing.
}
#endif  // NOT_PRODUCT
...
void MutableColoredSpace::ensure_parsability() {
  for (int i = 0; i < colored_spaces()->length(); i++) {
    ColoredSpace *ls = colored_spaces()->at(i);
...
size_t MutableColoredSpace::used_in_words() const {
  size_t s = 0;
  for (int i = 0; i < colored_spaces()->length(); i++) {
    s += colored_spaces()->at(i)->space()->used_in_words();
  }
  return s;
}

size_t MutableColoredSpace::free_in_words() const {
  size_t s = 0;
  for (int i = 0; i < colored_spaces()->length(); i++) {
    s += colored_spaces()->at(i)->space()->free_in_words();
  }
  return s;
}

size_t MutableColoredSpace::capacity_in_words(HeapColor color) const {
  guarantee(color >= HC_RED && color <= HC_TOTAL, "invalid color");
  if (color == HC_TOTAL) {
    if (colored_spaces()->length() > 0) {
      return capacity_in_words() / colored_spaces()->length();
    } else {
      assert(false, "There should be at least one colored space");
      return 0;
    }
  }
  int i = colored_spaces()->find(&color, ColoredSpace::equals);
  if (i == -1) {
    return 0;
  }
  return colored_spaces()->at(i)->space()->capacity_in_words();
}

#if 0
// Check if the Colored topology has changed. Add and remove spaces if needed.
// The update can be forced by setting the force parameter equal to true.
bool MutableColoredSpace::update_layout(bool force) {
...
// Color the region. Set the right page sizes.
void MutableColoredSpace::color_region(MemRegion mr, HeapColor color) {
...
void MutableColoredSpace::free_region(MemRegion mr) {
  HeapWord *start = (HeapWord*)round_to((intptr_t)mr.start(), page_size());
  HeapWord *end = (HeapWord*)round_down((intptr_t)mr.end(), page_size());
  if (end > start) {
    MemRegion aligned_region(start, end);
...
void MutableColoredSpace::update() {
  bool should_initialize = false;
  if (!os::numa_has_static_binding()) {
    for (int i = 0; i < colored_spaces()->length(); i++) {
      if (!colored_spaces()->at(i)->invalid_region().is_empty()) {
  ...
  if (should_initialize ||
      (ColoredUseAdaptiveChunkSizing && adaptation_cycles() < samples_count())) {
    // A Colored space is never mangled
    initialize(region(),
               SpaceDecorator::Clear,
               SpaceDecorator::DontMangle);
  }

  if (ColoredSpaceStats) {
    for (int i = 0; i < colored_spaces()->length(); i++) {
      colored_spaces()->at(i)->accumulate_statistics(page_size());
    }
  }

  scan_pages(ColoredSpacePageScanRate);
}

// Scan pages. Free pages that have smaller size or wrong placement.
void MutableColoredSpace::scan_pages(size_t page_count)
{
  size_t pages_per_chunk = page_count / colored_spaces()->length();
  if (pages_per_chunk > 0) {
    for (int i = 0; i < colored_spaces()->length(); i++) {
      ColoredSpace *cs = colored_spaces()->at(i);
      cs->scan_pages(page_size(), pages_per_chunk);
    }
  }
}

// Accumulate statistics about the allocation rate of each colored.
void MutableColoredSpace::accumulate_statistics() {
  if (ColoredUseAdaptiveChunkSizing) {
    for (int i = 0; i < colored_spaces()->length(); i++) {
      colored_spaces()->at(i)->sample();
    }
...
// chunk resizing.
size_t MutableColoredSpace::current_chunk_size(int i) {
  HeapWord *cur_end, *prev_end;
  if (i == 0) {
    prev_end = bottom();
  } else {
    prev_end = colored_spaces()->at(i - 1)->space()->end();
  }
  if (i == colored_spaces()->length() - 1) {
    cur_end = end();
  } else {
    cur_end = colored_spaces()->at(i)->space()->end();
  }
  if (cur_end > prev_end) {
...
// page_size() aligned.
size_t MutableColoredSpace::default_chunk_size(MutableColoredSpace *mcs, int i) {
#if 0
  size_t color_committed_space = 0;
  size_t extra = 0;

  if (mcs) {
    ColoredSpace *cs = mcs->colored_spaces()->at(i);
    MutableSpace *s = cs->space();

    HeapWord* committed_bottom = (HeapWord*)round_down((intptr_t) s->bottom(), page_size());
    HeapWord* committed_top = (HeapWord*)round_to((intptr_t) s->top(), page_size());
    color_committed_space = pointer_delta(committed_top, committed_bottom, sizeof(char));
  }
  extra = (base_space_size() - total_committed_space_size()) /
          colored_spaces()->length() * page_size();

  return color_committed_space + extra;
#endif
  return ((base_space_size() / colored_spaces()->length()) * page_size());
}

// Produce a new chunk size. page_size() aligned.
// This function is expected to be called on sequence of i's from 0 to
// colored_spaces()->length().
size_t MutableColoredSpace::adaptive_chunk_size(int i, size_t limit) {
  size_t pages_available = base_space_size();
  for (int j = 0; j < i; j++) {
    pages_available -= round_down(current_chunk_size(j), page_size()) / page_size();
  }
  pages_available -= colored_spaces()->length() - i - 1;
  assert(pages_available > 0, "No pages left");
  float alloc_rate = 0;
  for (int j = i; j < colored_spaces()->length(); j++) {
    alloc_rate += colored_spaces()->at(j)->alloc_rate()->average();
  }
  size_t chunk_size = 0;
  if (alloc_rate > 0) {
    ColoredSpace *cs = colored_spaces()->at(i);
    chunk_size = (size_t)(cs->alloc_rate()->average() / alloc_rate * pages_available) * page_size();
  }
...
void MutableColoredSpace::select_tails(MemRegion new_region, MemRegion intersection,
...
// |----------------|-------invalid---|--------------------------|
// |----bottom_region--|---intersection---|------top_region------|
void MutableColoredSpace::merge_regions(MemRegion new_region, MemRegion* intersection,
...
void MutableColoredSpace::resize_colored_space(MemRegion mr) {
  ShouldNotReachHere();
}

void MutableColoredSpace::move_red_to_blue() {
  ShouldNotReachHere();
}

void MutableColoredSpace::initialize(MemRegion mr,
                                  bool clear_space,
                                  bool mangle_space,
                                  bool setup_pages) {

  colored_initialize(mr, NULL, clear_space, mangle_space, setup_pages);
}

void MutableColoredSpace::colored_initialize(
  MemRegion mr,
  MutableColoredSpace *mcs,
  bool clear_space,
  bool mangle_space,
  bool setup_pages) {

  assert(clear_space, "Reallocation will destory data!");
  assert(colored_spaces()->length() > 0, "There should be at least one space");
  ...
  if (base_space_size_pages / colored_spaces()->length() == 0
  ...
  guarantee(base_space_size_pages / colored_spaces()->length() > 0, "Space too small");
  ...
  if (!old_region.equals(region())) {
    ...
    if (ApplyColorToSpaces) {
      if (_is_eden && EdenAlwaysRed) {
        color_region(bottom_region, HC_RED);
        color_region(top_region, HC_RED);
      } else {
        //HeapWord *start = (HeapWord*)round_to((intptr_t)bottom_region.start(), page_size());
        //HeapWord *end = (HeapWord*)round_down((intptr_t)bottom_region.end(), page_size());
        //HeapColor _color = colored_spaces()->at(0)->color();
        //tty->print_cr("  coloring bottom: {%p -- %p}, %d", start, end, _color);
        color_region(bottom_region, colored_spaces()->at(0)->color());

        //start = (HeapWord*)round_to((intptr_t)top_region.start(), page_size());
        //end = (HeapWord*)round_down((intptr_t)top_region.end(), page_size());
        //_color = colored_spaces()->at(colored_spaces()->length() - 1)->color();
        //tty->print_cr("  coloring top:    {%p -- %p}, %d", start, end, _color);
        color_region(top_region, colored_spaces()->at(colored_spaces()->length() - 1)->color());
      }
    }
  }

  size_t committed_space_size_pages = 0;
  if (mcs) {
    HeapWord *committed_bottom, *committed_top;
    for (int i = 0; i < mcs->colored_spaces()->length(); i++) {
      ColoredSpace *cs = mcs->colored_spaces()->at(i);
      MutableSpace *s = cs->space();

      committed_bottom = (HeapWord*)round_down((intptr_t) s->bottom(), page_size());
      committed_top = (HeapWord*)round_to((intptr_t) s->top(), page_size());
      committed_space_size_pages += (pointer_delta(committed_top,
                                      committed_bottom,
                                      sizeof(char)) / page_size());
    }
  }
  set_committed_space_size(committed_space_size_pages);
  ...
  bool layout_valid = ColoredUseAdaptiveChunkSizing          &&
                      current_chunk_size(0) > page_size() &&
                      current_chunk_size(colored_spaces()->length() - 1) > page_size();
#endif
  ...
  for (int i = 0; i < colored_spaces()->length(); i++) {
    ...
    if (i < colored_spaces()->length() - 1) {
      ...
      chunk_byte_size = default_chunk_size(mcs, i);
    }
    ...
    if (i < colored_spaces()->length() - 1) {
      if (!ColoredUseAdaptiveChunkSizing                                ||
          (ColoredUseAdaptiveChunkSizing && ColoredSpaceChunkResizeWeight == 0) ||
           samples_count() < AdaptiveSizePolicyReadyThreshold) {
        // No adaptation. Divide the space equally.
        chunk_byte_size = default_chunk_size();
      } else
        if (!layout_valid || ColoredSpaceResizeRate == 0) {
          ...
          // ColoredSpaceResizeRate bytes per collection.
          size_t limit = ColoredSpaceResizeRate /
                         (colored_spaces()->length() * (colored_spaces()->length() + 1) / 2);
    ...
    if (i == 0) { // Bottom chunk
      ...
      if (i < colored_spaces()->length() - 1) { // Middle chunks
        MutableSpace *ps = colored_spaces()->at(i - 1)->space();
...
#if 0
    if (!os::numa_has_static_binding()) {
      MemRegion invalid_region = cs->invalid_region().intersection(new_region);
      ...
      if (!invalid_region.is_empty()) {
        ...
        cs->set_invalid_region(MemRegion());
      }
    }
#endif
    ...
    if (ApplyColorToSpaces) {
      if (_is_eden && EdenAlwaysRed) {
        ...
        //HeapWord *start = (HeapWord*)round_to((intptr_t)bottom_region.start(), page_size());
        //HeapWord *end = (HeapWord*)round_down((intptr_t)bottom_region.end(), page_size());
        //HeapColor _color = cs->color();
        //tty->print_cr("  caloring bottom: {%p -- %p}, %d", start, end, _color);
        color_region(bottom_region, cs->color());

        //start = (HeapWord*)round_to((intptr_t)top_region.start(), page_size());
        //end = (HeapWord*)round_down((intptr_t)top_region.end(), page_size());
        //_color = cs->color();
        //tty->print_cr("  caloring top:    {%p -- %p}, %d", start, end, _color);
        color_region(top_region, cs->color());
      }
    }
...
/* MRJ -- check this one -- it calls os::numa_has_static_binding */
// Set the top of the whole space.
// Mark the the holes in chunks below the top() as invalid.
void MutableColoredSpace::set_top(HeapWord* value) {
  bool found_top = false;
  for (int i = 0; i < colored_spaces()->length();) {
    ColoredSpace *cs = colored_spaces()->at(i);
    MutableSpace *s = cs->space();
    ...
    if (s->contains(value)) {
      ...
      if (!os::numa_has_static_binding() && top < value && top < s->end()) {
        cs->add_invalid_region(MemRegion(top, value));
      }
#endif
      s->set_top(value);
      found_top = true;
        ...
        if (found_top) {
          ...
          if (!os::numa_has_static_binding() && top < s->end()) {
            cs->add_invalid_region(MemRegion(top, s->end()));
          }
#endif
          s->set_top(s->end());
        }
...
void MutableColoredSpace::clear(bool mangle_space) {
  MutableSpace::set_top(bottom());
  for (int i = 0; i < colored_spaces()->length(); i++) {
    // Never mangle Colored spaces because the mangling will
    // bind the memory to a possibly unwanted lgroup.
    colored_spaces()->at(i)->space()->clear(SpaceDecorator::DontMangle);
  }
}
...
HeapWord* MutableColoredSpace::allocate(size_t size, HeapColor color) {
#if 0
  ...
  int i = lgrp_spaces()->find(&lgrp_id, ColoredSpace::equals);
...
#if 0
  if (color == HC_BLUE)
    tty->print("allocating blue: old_top: %p ", top());
#endif

  ColoredSpace* cs = colored_spaces()->at(color);
  MutableSpace *s = cs->space();
...
HeapWord* MutableColoredSpace::cas_allocate(size_t size, HeapColor color) {
#if 0
  Thread* thr = Thread::current();
  int lgrp_id = thr->lgrp_id();
  if (lgrp_id == -1 || !os::numa_has_group_homing()) {
...
void MutableColoredSpace::print_short_on(outputStream* st) const {
  MutableSpace::print_short_on(st);
  st->print(" (");
  for (int i = 0; i < colored_spaces()->length(); i++) {
    st->print("color %d: ", colored_spaces()->at(i)->color());
    colored_spaces()->at(i)->space()->print_short_on(st);
    if (i < colored_spaces()->length() - 1) {
      st->print(", ");
    }
  }
...
void MutableColoredSpace::print_on(outputStream* st) const {
  MutableSpace::print_on(st);
  for (int i = 0; i < colored_spaces()->length(); i++) {
    ColoredSpace *cs = colored_spaces()->at(i);
    st->print("    color %s", heapColor2Str(cs->color()));
    cs->space()->print_on(st);
    if (ColoredSpaceStats) {
      for (int i = 0; i < colored_spaces()->length(); i++) {
        colored_spaces()->at(i)->accumulate_statistics(page_size());
      }
      st->print("    local/remote/unbiased/uncommitted: %zuK/%zuK/%zuK/%zuK, large/small pages: %zu/%zu\n",
                cs->space_stats()->_local_space / K,
                cs->space_stats()->_remote_space / K,
                cs->space_stats()->_unbiased_space / K,
                cs->space_stats()->_uncommited_space / K,
                cs->space_stats()->_large_pages,
                cs->space_stats()->_small_pages);
    }
  }
}

void MutableColoredSpace::verify() {
...
void MutableColoredSpace::ColoredSpace::accumulate_statistics(size_t page_size) {
  clear_space_stats();
  ...
  if (start < end) {
    for (char *p = start; p < end;) {
      os::colored_page_info info;
      if (os::get_colored_page_info(p, &info)) {
        if (info.size > 0) {
...
void MutableColoredSpace::ColoredSpace::scan_pages(size_t page_size, size_t page_count)
{
  ...
  os::colored_page_info page_expected, page_found;
  page_expected.size = page_size;
  page_expected.color = color();

  char *s = scan_start;
  while (s < scan_end) {
    char *e = os::scan_colored_pages(s, (char*)scan_end, &page_expected, &page_found);
    if (e == NULL) {
      break;
    }
    if (e != scan_end) {
      if ((page_expected.size != page_size || page_expected.color != color())
